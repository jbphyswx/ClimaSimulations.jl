...
"""
    member_values(interface, iteration, case, var, levels)

Every member's scored values for one variable at one iteration, restricted to `levels` and
averaged over the case's scoring window. Stops at the first member whose output is absent.
"""
function member_values(
    interface::SC.SOCRATESInterface,
    iteration::Integer,
    ...
    member = 1
    while isdir(SC.case_output_dir(interface, iteration, member, c))
        dir = SC.case_output_dir(interface, iteration, member, c)
        restricted = SC.restrict_to_levels(SOCRATES.run_outputvars(dir, (var,))[var], levels)
        push!(values, vec(Array{Float64}(SC.windowed_time_mean(restricted, window).data)))
        member += 1
    end
    return values
end
...
"""
    calibration_figures(interface, prior; output_dir, iterations, profile_vars, path_vars, logscale)

Write every figure a finished calibration supports, returning the paths written.

The parameter and misfit figures need only the EKP files. The per-case profile and water-path
figures read each member's run output, which `prune_output = true` deletes after every
iteration, so those are skipped with a warning when it is not there.
"""
function calibration_figures(
    ...
    logscale::Bool = true, # should just be normal kwargs to CairoMakie
)
    ...
    for c in interface.cases
        name = SOCRATES.case_name(c)
        window = SC.score_window(c)
        levels = SC.scored_levels(
            SOCRATES.socrates_z(SC.case_grid(interface, c)), SC.default_z_bounds(c),
        )
        have = [it for it in wanted if isdir(SC.case_output_dir(interface, it, 1, c))]
        if isempty(have)
            @warn "No member output for $name; skipping its profile and path figures. \
                   Build the interface with `prune_output = false` to keep it."
            continue
        end
        ...
        for var in profile_vars
            r = SC.windowed_time_mean(
                SC.reference_on_levels(reference[var], levels), window,
            )
            reference_by_var[var] = vec(Array{Float64}(r.data))
            groups_by_var[var] = [
                "iteration $it" => member_values(interface, it, c, var, levels) for
                it in have
            ]
        end
        push!(
        ...
        reference_paths =
            [only(SC.windowed_time_mean(reference[v], window).data) for v in path_vars]
        path_groups = map(have) do it
            per_var = [member_values(interface, it, c, v, levels) for v in path_vars]
        ...
        push!(
            ...
            scalar_comparison(
                ...
                path = joinpath(output_dir, "$(name)_paths.png"),
