...
"""
    member_values(interface, iteration) -> (; labels, columns)

Every member's `G` column at one iteration, with one label per row. Stops at the first member
whose output is absent.
"""
function member_values(interface::CBC.CloudBenchInterface, iteration::Integer)
    labels = String[]
    columns = Vector{Float64}[]
    member = 1
    while isdir(
        CBC.member_output_dir(interface, iteration, member, first(interface.cases)),
    )
        rows = CBC.member_scored_rows(interface, iteration, member)
        isempty(labels) && (labels = first.(rows))
        push!(columns, collect(Float64, last.(rows)))
        member += 1
    end
...
The parameter and misfit figures need only the EKP files. Every scored CloudBench variable is a
time mean rather than a profile, so the ensemble comparison is one scalar panel over all
`(case, variable)` pairs; the profile figures the other cases use do not apply here.
"""
function calibration_figures(
    ...
    have = [
        it for it in wanted if
        isdir(CBC.member_output_dir(interface, it, 1, first(interface.cases)))
    ]
    if isempty(have)
        @warn "No member output under $(interface.output_dir); skipping the scored-value figure."
        return written
    end
    by_iteration = Dict(it => member_values(interface, it) for it in have)
    labels = by_iteration[first(have)].labels
    push!(
        written,
        scalar_comparison(
            labels,
            collect(Float64, EKP.get_obs(ekp)),
            ["iteration $it" => by_iteration[it].columns for it in have];
            path = joinpath(output_dir, "scored_values.png"),
            title = "scored values",
            reference_label = "CloudBench LES",
        ),
    )
    return written
end
