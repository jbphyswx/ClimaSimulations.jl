...
"""
topology_key(grid) = length(cloudbench_z(grid))
...
"""
    run_cases(cases; FT, output_dir, executor, grid, kwargs...)
...
function run_cases(
    ...
    run_one =
        c -> run_case(
            ...
            FT,
            grid,
            output_dir = joinpath(output_dir, cloudbench_job_id(c)),
            kwargs...,
        )
    return run_tasks(
        run_one,
        cases,
        fill(topology_key(grid), length(cases)),
        executor,
    )
end
